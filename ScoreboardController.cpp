...
void ScoreboardController::draw_shooting_indicator(int x, bool onRed)
{
	...
	if(gameState == 1){
		...
		int shootingPlayer = snookerGame->shooting_player();
...
{	
	...
	if(pointDiff >= 0 && pointDiff > remainingPoints){
		...
		draw_right_aligned(points[1], 30, 3, 128, 128, 128);
	} else {
	...
	draw_right_aligned(breaks[1], 30, 13, 128, 128, 128);
	...
	draw_right_aligned(pointsOnTable, 30, 23, 128, 128, 128);
}
